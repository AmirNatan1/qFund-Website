# qFund website content source ledger

Last reviewed: 23 July 2026

This release replaces the previous provisional website copy. Visible investment, team, portfolio, and platform claims are grounded in the three qFund-supplied decks below or in the linked public sources. Navigation labels and interface instructions are functional website language.

## Source priority

1. `_qFund Overview - May 26 (For Gal).pdf` — title slide dated June 2026; used as the newest internal source.
2. `_qFund - VC Delegation to Japan Nov 2025.pdf` — used for qFund II, strategic focus, and deal-flow statements.
3. `Q Fund Portfolio.pdf` — used to corroborate the portfolio roster and company descriptions.
4. Official company websites, the Israel Innovation Authority, Startup Nation Central, the Embassy of Israel in Japan, qFund’s public pages, and the supplied LinkedIn profiles.

## Claim map

| Website content | Primary source |
| --- | --- |
| “Funding the deep future of technology” | Japan deck, p. 1 |
| Early-stage VC backing Deep Tech founders | Japan deck, p. 3; overview deck, p. 10 |
| Core infrastructure, hardware, enabling technologies, and named sectors | Japan deck, p. 3; overview deck, p. 10 |
| Financial investment, technical validation, commercialization support, and strategic access | Japan deck, p. 3; overview deck, p. 10 |
| Liav Ben Rubi and Dana Taigman Koren biographies | Overview deck, p. 11 |
| Team titles, including Liron Ben Zaken as Principal | [qFund public website](https://qfund.io/); [qFund public LinkedIn](https://www.linkedin.com/company/q-fund) |
| qFund × Quantum Hub integrated growth platform | Japan deck, pp. 8–9; overview deck, pp. 12–14 |
| Three-year deal-flow funnel | Japan deck, p. 10 |
| Q Factor: founders, Deep Tech, and market | Japan deck, p. 11; overview deck, pp. 18–23 |
| qFund II strategic focus areas | Japan deck, p. 12 |
| Four-pillar Deep Tech evaluation method | Overview deck, p. 45 |
| Portfolio roster and logos | Overview deck, pp. 15–17; portfolio deck, pp. 1–2 |
| Company descriptions, founding years, and stages | Overview deck, pp. 25–47; portfolio deck, pp. 4–23 |

## Portfolio links

Every portfolio logo links directly to the best verified public company URL available.

| Company | Website verification |
| --- | --- |
| Element Security | [Official website](https://element.security/) |
| Commcrete | [Official website](https://www.commcrete.com/) |
| Skapion | [Official website](https://www.skapion.com/) |
| Oraqon | [Official website](https://www.oraqon.com/) |
| Qedma | [Official website](https://www.qedma.com/) |
| Actasys | [Official website](https://www.actasysinc.com/) |
| Particle | [Official holding page](https://particle-lab.com/) |
| Signal Edge | [Official website](https://signal-edge.com/); [Israel Innovation Authority](https://innovationisrael.org.il/importWinner/516999810/) |
| LiteVision | [Official website](https://litevision-eo.com/); [Israel Innovation Authority](https://innovationisrael.org.il/en/importWinner/517155560/) |
| QuamCore | [Official website](https://www.quamcore.com/) |

## Team image and profile links

The portraits are qFund’s own published team images. Each portrait links to the LinkedIn profile supplied by the website owner.

| Team member | Portrait source | LinkedIn |
| --- | --- | --- |
| Liav Ben Rubi | `qfund.io/wp-content/uploads/2024/09/Untitled-2.webp` | [Profile](https://www.linkedin.com/in/liav-ben-rubi/) |
| Dana Taigman Koren | `qfund.io/wp-content/uploads/2024/09/Untitled-1.webp` | [Profile](https://www.linkedin.com/in/danataigmankoren/) |
| Liron Ben Zaken | `qfund.io/wp-content/uploads/2026/03/Liron-omain-Manager-Energy-industry-4.0-2-1.png` | [Profile](https://www.linkedin.com/in/liron-ben-zaken/) |

## Reconciliations

- One overview slide says “11 cutting edge companies,” while the current roster slides and logo grid name ten companies. The website uses the ten companies that are individually named and documented.
- Earlier materials included Indoor Robotics; the newer June 2026 overview replaces it in the displayed roster with Skapion. The website follows the newer roster.
- Legacy qFund copy dated Element Security to 2022. Element Security’s public company sources date its founding to 2021, so the website uses 2021.
- Financial and valuation figures in the supplied decks are intentionally excluded from the public website.
