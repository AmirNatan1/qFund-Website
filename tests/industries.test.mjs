...
test("preloads every scene into one warmed WebGL renderer before industry scrolling", async () => {
  ...
  assert.match(experience, /--qf-industry-active-shift/);
  assert.match(experience, /nextIndex - chapterPosition/);
  assert.match(experience, /PORTAL_PROGRESS/);
  assert.match(experience, /--qf-industry-portal-scale/);
  assert.match(experience, /--qf-industry-portal-opacity/);
  ...
  assert.match(styles, /translate3d\(var\(--qf-industry-active-shift\), 0, 0\)/);
  assert.match(styles, /height: calc\(var\(--qf-industry-count\) \* 100svh \+ 100svh\)/);
